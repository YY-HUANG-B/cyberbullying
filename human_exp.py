...
def get_therapist_system_prompt():
    ...
    if current_round == 1:
        ...
        **应对策略**：
        ...
        # 检测上一轮是否举例，如果有则跳过"具体案例示范"策略
        last_therapist_contents = st.session_state.get('last_3_therapist_contents', [])
        last_round_has_example = False
        example_keywords = ["有个学生", "有个青少年", "我遇到一个", "我之前遇到", "有个男生", "有个叫", "记得有个人", "有个人叫"]
        if last_therapist_contents:
            for msg in last_therapist_contents[:1]:  # 只检查上一轮
                if any(kw in msg for kw in example_keywords):
                    last_round_has_example = True
                    break
        ...
        # 如果上一轮举过例，跳过"具体案例示范"策略
        variant_idx = (current_round - 5) % len(strategy_variants)
        if last_round_has_example and variant_idx == 1:  # idx=1是"具体案例示范"
            variant_idx = (variant_idx + 1) % len(strategy_variants)

        variant_title, variant_focus, variant_example = strategy_variants[variant_idx]

        # 如果上一轮举过例，额外提醒本轮不要举例
        example_warning = ""
        if last_round_has_example:
            example_warning = """
        **【本轮禁止举例】**：上一轮已经举过例子，本轮绝对不能再讲任何案例故事。
        请用其他方式展开论述，比如直接分析、给建议、提问等。
        """
    ...
    **处理方式**：
    ...
    if last_3:
        ...
        # 检测被试上一轮的内容关键词，避免重复引用被试已不再提的内容
        last_bully_content = ""
        for msg in reversed(st.session_state.conversation_history):
            if msg["role"] == "Bully":
                last_bully_content = msg.get("content", "")
                break
...
**【禁止重复引用被试旧内容】**：
- 被试**上一轮**说的是："{last_bully_content[:100] if last_bully_content else '无'}"
- 只回应被试**本轮**说的内容，不要翻旧账引用被试好几轮前说过的话
- 如果被试本轮只是简短回应（如"可以的"、"我同意"），不要强行把话题扯回之前的内容
- 被试不再提的事情，你也不应该反复提，这是基本的对话礼貌
...
**差异化的具体方法**：
1. 换角度：上一轮讲方法，这轮可以直接分析、提问或肯定，不要总想着讲故事
2. 换句式：避免"我想到一个..."、"有一个情况是..."等举例开头的句式
3. 换内容：直接回应被试本轮说的话，而不是扯到别的话题或编造故事
4. 换语气：上一轮温和这轮可以更直接，或反过来
    ...
    **回复内容禁止事项**：
    ...
    4. **【严禁自我注释】**：禁止在回复中添加括号说明，如"（注意：这是...）"、"（这是用...方式）"等
    5. **【严格控制举例频率】**：
       - 【强制】每3轮最多举1次例子，连续两轮绝不能都举例
       - 禁止使用"我记得有个青少年叫XX"、"我之前遇到一个学生"、"有个男生叫XX"这种格式化的虚假例子
       - 如果一定要举例，必须是简短自然地融入论述，不要专门讲一个"故事"
       - 【推荐】与其编造例子，不如直接分析对方的话或给出具体可操作的建议
    6. **【标点符号强制规范】**：
       - 【强制】每个完整句子必须以句号结尾，没有任何例外
       - 【强制】禁止出现连续3个以上的短句不加句号的情况
       - 正确使用逗号分隔子句，问句结尾用问号
       - 示例错误："我理解你的感受这让我想到一个故事后来他改变了"
       - 示例正确："我理解你的感受。这让我想到，其实很多人都有类似的经历。"
       - 【检查清单】输出前必须检查：每段话是否都有句号？
    7. **【禁止重复引用被试旧内容】**：
       - 只回应被试**本轮**说的内容，不要翻旧账引用被试好几轮前说过的话
       - 如果被试本轮只是简短回应（如"可以的"、"我同意"），不要强行把话题扯回之前的内容
       - 被试不再提的事情，你也不应该反复提，这是基本的对话礼貌
       - 例如：被试之前说过"气不过又放不下脸面"，但如果被试已经不再提这个了，你就不应该连续几轮都引用这句话
